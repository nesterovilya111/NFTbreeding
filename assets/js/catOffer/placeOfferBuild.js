...
function eyeVariation(num, id) {
    ...
    switch (num) {
        case '1':
            normalEyes(id)
            $('#eyeName').html('Eyes: Basic')
            break
        case '2': 
            normalEyes(id)
            $('#eyeName').html('Eyes: Chill')
            SleepyEyes(id)
            break
        case '3':
            normalEyes(id)
            $('#eyeName').html('Eyes: Thinker')
            lookingUp(id)
            break
        case '4':
            normalEyes(id)
            $('#eyeName').html('Eyes: Blank')
            blankEyes(id)
            break
        default: 
            normalEyes(id)
...
function patternVariation(num, id) {
    ...
    switch (num) {
        case '1':
            $('#patternName').html('Pattern: Basic')
            normalpattern(id)
            break
        case '2': 
            normalpattern(id)
            $('#patternName').html('Pattern: Third Eye')
            firstpattern(id)
            break
        case '3': 
            normalpattern(id)
            $('#patternName').html('Pattern: Swave')
            secondpattern(id)
            break
        case '4': 
            normalpattern(id)
            $('#patternName').html('Pattern: Femmy')
            thirdpattern(id)
            break
    }
}
...
function animationVariation(num, id){
    ...
    switch(num){
        case '1':
            resetAnimations(id);
            $('#animationName').html('Animation: Still')
            break;
        case '2':
            resetAnimations(id);
            $('#animationName').html('Animation: Headshake')
            animationType1(id)
            break;
        case '3':
            resetAnimations(id);
            $('#animationName').html('Animation: Ignite')
            animationType2(id)
            break;
        case '4':
            resetAnimations(id);
            $('#animationName').html('Animation: Shaky tail ')
            animationType3(id)  
            break; 
    }
}
